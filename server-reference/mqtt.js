...
class MQTTClient {
  ...
  async storeSensorData(data) {
    try {
      ...
      await firebase.database().ref(`${sensorPath}/${readingId}`).set({
        ...data,
        processedAt: Date.now()
      });
      
      // Also update latest reading
      await firebase.database().ref(`sections/${data.sectionId}/latest`).set({
        ...data,
        processedAt: Date.now()
      });
  ...
  async storeRecommendation(sectionId, recommendation) {
    try {
      ...
      await firebase.database().ref(recommendationPath).set({
        ...recommendation,
  ...
  async publishCarControl(action) {
    ...
    try {
      ...
      await this.publish(this.topics.carControl, JSON.stringify(controlMessage));
  ...
  async publishSystemStatus(status, reason = '') {
    ...
    try {
      await this.publish(this.topics.systemStatus, JSON.stringify(statusMessage), { retain: true });
